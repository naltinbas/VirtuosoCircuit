...
Every row below comes from `src/licensing/AssetManifest.ts`, the same list the in-game credits screen is built from, which in turn takes its track rows from the metadata in `src/charts/tracks/`. The credits screen is generated at runtime; this file is not. If the two ever disagree, the metadata is right and this file has fallen behind.

Two claims are kept apart throughout this file, because they are not the same claim:

- The **composition** is a piece of music written by somebody who has been dead long enough for it to be in the public domain.
- The **arrangement** is the note-event data in this repository, written for this game.

A composition being in the public domain does not make any particular recording of it free to use. A recording carries its own rights, held by the performers and whoever produced it, and those are separate from and usually much younger than the rights in the score. This project sidesteps the question entirely: it uses no recordings.
...
"Attribution required" is a legal question, and the answer is no for every row: a public-domain composition imposes no duty to credit, and everything else in the table was made for this project. The composers are named anyway, in this file and on the credits screen, because it is worth knowing whose music you are playing.

## Source notes

Each note below is the `scoreSourceCredit` from that track's module. They say which bars are quoted from the score and which were written for the arrangement, because "arranged from" and "written in the character of" are different things and a reader deserves to know which they are hearing.
...
Every piece played in this game is a public-domain composition. The composers, in catalog order:
...
- The arrangements are typed out as beats and pitches in `src/charts/tracks/`, in the notation described in `docs/chart-format.md`. They are source code, they are covered by the project licence in `LICENSE`, and they can be read, diffed and edited like any other file here.
...
- The charts (which lanes you press, and when) are also original work, written against the arrangements rather than derived from anything.
- Sound is produced by oscillators, noise buffers, filters and gain envelopes built at runtime in `src/audio/SynthInstruments.ts`. There are no samples, no sample libraries and no recorded instruments anywhere in this project.
...
There are no external links in this file. Verifying a public-domain claim properly means checking a source that has been looked at, and no such check has been run from this machine, so nothing is cited that could not be confirmed. Anyone reviewing these claims should look up the composers' dates and the catalogue numbers themselves rather than trusting a URL pasted in here.
