...
| Logo and icon | Not applicable | Original artwork drawn for this game | Hand-written SVG in this repository. The lettering is constructed geometry on a 100 unit cap grid, not an outlined typeface, so no font file is involved. | Not applicable | Part of this project's own source. No third-party rights are involved. | No |
...
- The logo and the icon are hand-written SVG: `logo.svg` is the banner, `logo-plain.svg` the same lockup without its panel, `public/mark.svg` the mark on its own and `public/favicon.svg` a simplified mark that still reads at 16 px. The wordmark is built from circles, arcs and straight segments on a 100 unit cap grid rather than from an outlined typeface, so no font file is involved in the artwork either. The interface uses the system font stack declared as `--font-stack` in `src/ui/styles.css`, so no font file is bundled or downloaded.
